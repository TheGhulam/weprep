...
def get_most_used_words(transcript_data, n=10):
    exclude_words = ["i", "we", "they", "a", "an", "the", "and", "or", "but", "is", "are", "was", "were", "be", "been",
                     "you", "it", "he", "she", "they", "we", "me", "him", "her", "us", "them", "my", "your", "his",
                     "to", "of", "in", "on", "at", "by", "for", "with", "as", "from", "into", "onto"]
    # Extract all "pronunciation" words
    pronunciation_words = [item["alternatives"][0]["content"].lower()
...
def calculate_speech_speed(srt_data: str) -> float:
    ...
    text_lines = [line.strip() for line in lines if '-->' not in line and not line.isdigit()]
...
def count_hedging_words(transcript_data: dict) -> int:
    ...
    """
    hedging_words = ["maybe", "perhaps", "possibly", "probably", "likely", "apparently", "seems", "appears", "looks",
                     "think", "believe", "suppose", "guess", "sort", "kind", "somewhat", "rather", "fairly",
                     "relatively", "somewhat", "about", "around", "approximately", "almost", "nearly", "roughly",
                     "like"]
    ...
    hedging_word_count = sum(re.search(rf"\b{word}\b", transcript_text) is not None for word in hedging_words)
...
def count_filler_words(transcript_data: dict) -> tuple[list[str], int]:
    ...
    """
    filler_words = ["um", "uh", "like", "you know", "so", "basically", "right", "well", "actually"]
